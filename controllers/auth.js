...
async function signUp(req, res, next) {
    try {
        ...
        if (type === "doctor") {
            ...
            const res_object = { message: `successfuly signup doctor ${doctor.name} `, success: true, result: doctor, notify: true };
            ...
            const res_object = { message: `successfuly signup patient ${patient.name} `, success: true, result: patient, notify: true };
            req.res_object = res_object;
            next()
...
async function signIn(req, res, next) {
    try {
        const { body } = req;
        const { type, email, password, name } = body;
        if (!email || !password) throw new Error('prameter missed,please enter all required fields ')
        if (type === "doctor") {
            ...
            if (comparePassword(password, doctor.password)) {
                ...
                const res_object = { message: `successfuly signin`, success: true, results: [createdToken, doctor], notify: true };
                req.res_object = res_object;
                return next()
            }
        ...
        if (type === "patient") {
            ...
            if (comparePassword(password, patient.password)) {
                ...
                const res_object = { message: `successfuly signin`, success: true, results: [createdToken, patient], notify: true };
                req.res_object = res_object;
                return next()
            }
            throw new Error('email or password incorrect');
        ...
        throw new Error(`Error has occured mayabe dosent choose type  !`)

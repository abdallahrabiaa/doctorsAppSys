...
async function randomAppointment(req, res, next) {
    try {
        ...
        if (!doctors && !doctors.length) throw new Error(`Could not find any doctors for ${category}`);
        if (!patient && ! await PATIENT.isExist(patient)) throw new Error(` no Patient found`)
        const doctorIndex = Math.floor(Math.random() * doctors.length);
        const doctor = doctors[doctorIndex];
        const appointment = await APP.create({ doctor: doctor._id, patient: patient, cost: doctor.AppointmentCost, date, online })
        const res_object = { message: "successfuly retrive data", success: true, result: appointment, notify: false };
...
router.post(modelPath + "-random", (req, res, next) => {
    req.Model = APP,
        req.model_name = modalName
    next()
}, randomAppointment, response)
